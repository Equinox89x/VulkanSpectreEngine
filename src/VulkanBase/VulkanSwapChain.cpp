...
namespace Spectre {
    ...
    VulkanSwapChain::~VulkanSwapChain()
	{
        ...
        for (int index = 0; index < m_DepthImages.size(); index++) 
        {
            vkDestroyImageView(m_Device.GetDevice(), m_DepthImageViews[index], nullptr);
            vkDestroyImage(m_Device.GetDevice(), m_DepthImages[index], nullptr);
            vkFreeMemory(m_Device.GetDevice(), m_DepthImageMemorys[index], nullptr);
        }
        ...
        {
            vkDestroySemaphore(m_Device.GetDevice(), m_RenderFinishedSemaphores[index], nullptr);
            vkDestroySemaphore(m_Device.GetDevice(), m_ImageAvailableSemaphores[index], nullptr);
            vkDestroyFence(m_Device.GetDevice(), m_InFlightFences[index], nullptr);
        }
    }
    ...
    void VulkanSwapChain::CreateImageViews()
	{
        ...
        for (size_t index = 0; index < m_Images.size(); index++) 
        {
            ...
            if (vkCreateImageView(m_Device.GetDevice(), &viewInfo, nullptr, &m_ImageViews[index]) != VK_SUCCESS) 
            {
    ...
    void VulkanSwapChain::CreateFramebuffers()
	{
        ...
        for (size_t index = 0; index < GetImageCount(); index++) 
        {
            std::array<VkImageView, 2> attachments = { m_ImageViews[index], m_DepthImageViews[index] };
    ...
    void VulkanSwapChain::CreateDepthResources()
	{
        ...
        for (int index = 0; index < m_DepthImages.size(); index++) 
        {
            ...
            m_Device.CreateImageWithInfo(imageInfo,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,m_DepthImages[index],m_DepthImageMemorys[index]);
            ...
            if (vkCreateImageView(m_Device.GetDevice(), &viewInfo, nullptr, &m_DepthImageViews[index]) != VK_SUCCESS) 
            {
                throw std::runtime_error("failed to create texture image view!");
            }
        }
